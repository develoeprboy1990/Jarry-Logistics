    ...
    <div class="card-header bg-transparent border-bottom">
      ...
      <div class="card-body">
        ...
        <form action="{{URL('/SalesmanSave')}}" method="post"> {{csrf_field()}} 
          ...
          <input type="text" class="form-control" name="SalesmanName" value="{{old('SalesmanName')}}" required="">
          </div>
          </div>
...
<div class="card">
    <div class="card-body">
      ...
      @foreach ($salesman as $key =>$value)
       ...
       <td class="col-md-5">{{$value->SalesmanName}}</td>
       <td class="col-md-1">{{$value->Mobile}}</td>
       <td class="col-md-3">{{$value->Address}}</td>
       <td class="col-md-1"><a href="{{URL('/SalesmanEdit/'.$value->SalesmanID)}}"><i class="bx bx-pencil align-middle me-1"></i></a> <a href="#" onclick="delete_confirm2('SalesmanDelete',{{$value->SalesmanID}})"><i class="bx bx-trash  align-middle me-1"></i></a></td>
       </tr>
       @endforeach   
       </tbody>
       </table>
